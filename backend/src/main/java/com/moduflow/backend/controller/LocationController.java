...
public class LocationController {
    ...
    @PostMapping({"/update-location", "/v1/update-location"})
    @Operation(
    ...
    public ResponseEntity<LocationUpdateResponse> updateLocation(@RequestBody LocationRequest request) {
        return ResponseEntity.ok(locationService.updateLocation(request));
    }

    @GetMapping({"/current-location/{userId}", "/v1/current-location/{userId}"})
    @Operation(
